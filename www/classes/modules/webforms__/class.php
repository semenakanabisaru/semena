...
class webforms extends def_module {
	...
	public function getEditLink($element_id, $element_type) {
		return array(false, '/admin/content/edit/'.$element_id.'/');
	}
	...
	public function checkRequiredFields($typeId) {
		...
		$allFields   = umiObjectTypesCollection::getInstance()->getType($typeId)->getAllFields();
		$errorFields = array();
			...
			foreach($data as $field => $cont) {
				if($filename = $_FILES['data']['name'][$field]) {
					...
					$someMail->attachFile($file);
						...
						$cont = templater::putLangsStatic("%not_enough_space_for_load_file%");
					}
				}
...
END;
		...
		if($template = (string) $_REQUEST['template']) {    //Sending auto-reply
			list($template_mail, $template_mail_subject) = def_module::loadTemplates("webforms/".$template, "webforms_reply_mail", "webforms_reply_mail_subject");

			$template_mail = def_module::parseContent($template_mail, $arr);
			$template_mail_subject = def_module::parseContent($template_mail, $arr);
			$check_param = false;
