	...
	class matches implements iMatches {
		...
		public function __construct($sitemapFileName = "sitemap.xml") {
			...
			if (is_null($sitemapFilePath)) {
				$sitemapFilePath = CURRENT_WORKING_DIR . "/umaps/" . $sitemapFileName;
				// Сначала пытаемся получить sitemap из ресурсов, затем по дефолту
				if ($resourcesDir = cmsController::getInstance()->getResourcesDirectory()) {
					$sitemapFilePath = $resourcesDir . "/umaps/" . $sitemapFileName;
					if (!is_file($sitemapFilePath)) {
						$sitemapFilePath = CURRENT_WORKING_DIR . "/umaps/" . $sitemapFileName;
					}
				}
			}
