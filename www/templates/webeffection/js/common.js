	...
	function changePass(forms) {
		...
		$.ajax({
  			...
  			success: function(msg) {
  				if (msg.result == '0') {
  					$(forms).find('span.button').after('<span class="report">Неверный пароль</span>');
	...
	$(document).ready(function() {
        ...
        var invisible_fancy_gallery = $('#invisible_fancy > ul > li > a');
		$('.zoom_img').bind('click', function(e) {
             e.preventDefault();
             var src = $(this).attr('href');
             var $link = invisible_fancy_gallery.filter('a[href="'+src+'"]');
             $link.trigger('click');
        });
	    ...
	    $("#photos_carousel li img").click(function() {
            ...
            $(image).find('.zoom_img').bind('click', function(e) {
                e.preventDefault();
                var src = $(this).attr('href');
                var $link = invisible_fancy_gallery.filter('a[href="'+src+'"]');
                $link.trigger('click');
