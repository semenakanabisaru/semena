...
/**
	...
	this.upload = function(files) {
		/////umiru
		if(files && maxFileCountAllowed) {
			var count = 0;
			if(files.input) {
				if (files.input.files) {
					count = files.input.files.length;
				}
			} else if(files.files) {
				count = files.files.length;
			}
			if(count > maxFileCountAllowed) {
				this.error("Одновременно загружать можно не более " + maxFileCountAllowed + " файлов");
