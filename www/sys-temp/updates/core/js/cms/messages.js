function umiMessages () {
	...
	this.render = function (info) {
		var create = function (tag) { return jQuery(document.createElement(tag)); };
		var append = function (child, parent) { return jQuery(parent).appendChild(jQuery(child)); };
		...
		jQuery(header).update(info.title);
		jQuery(content).update(info.content);
		jQuery(sender).update(dtStr + " - " + info['sender-login']);
