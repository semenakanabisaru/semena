	...
	abstract class __users extends baseModuleAdmin {
		...
		public function edit() {
			...
			if($mode == "do") {
				...
				if(isset($_REQUEST['data'][$objectId]['login'])) {
					$otherUserId = $this->checkIsUniqueLogin($_REQUEST['data'][$objectId]['login'], $objectId);
					if($otherUserId !== true && $otherUserId != $objectId) {
		...
		public function onCreateObject($e) {
			...
			if($e->getMode() == "before") {
				...
				if(!$this->checkIsUniqueLogin((string) $_REQUEST['data']['new']['login'])) {
