	...
	class umiInstallExecutor {
		...
		*/
		private function ht_create_dummy($fulldummyname) {
			...
			$dummy = "<?php header(\"Content-Type: text/html; charset=utf-8\"); ?>\n".$dummy;
	...
	/** Операции по восстановлению системы */
		...
		*/
		private function restoreStructure() {
			...
			$this->includeMicrocore();
			$converter = new dbSchemeConverter($this->connection, $backupfilename);
...
//					$path->appendChild($xml->createTextNode($one));
		...
		*/
		private function runTests() {
			...
			$test_mode = ($this->install_mode?'install':'migration');

			$tests = new testHost();
			$tests->setConnect($host, $login, $password, $dbname, $test_mode);
			$tests->getResults();
		...
		*/
		private function updateDatabaseStructure() {
			...
			///TODO Продумать, как правильно формировать install.ini в режиме обновления
			...
			if ($this->install_mode) {
				...
				$xmlObj = simplexml_load_file('./sys-temp/updates/core/smu/database.xml');
				$tables = $xmlObj->xpath("table");
				foreach($tables as $table) {
					l_mysql_query("DROP TABLE IF EXISTS `".((string)$table->attributes()->name)."`");
					$this->flushLog("Очистка таблицы ".((string)$table->attributes()->name));
				}
