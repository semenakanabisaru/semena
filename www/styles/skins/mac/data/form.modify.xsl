...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="group[position() = 1 and count(../../basetype)]" mode="form-modify-group-fields">
		...
		<div class="extended_fields">
			...
			<xsl:apply-templates select="//page/copies" mode="form-modify" /><br />
		</div>
		<xsl:apply-templates select="field[@type='wysiwyg' or @type='text']" mode="form-modify" />
	</xsl:template>
	
	<xsl:template match="copies/copy" mode="form-modify">
		<xsl:apply-templates select="parents/item" mode="form-modify" />
		<xsl:choose>
	...
	<xsl:template match="parents/item" mode="form-modify">
		<a href="/admin/{./@module}/{./@method}/" target="_blank" class="tree_link" 
			onclick="javascript:return treeLink('{./@settingsKey}', '{./@treeLink}');">
			<xsl:value-of select="./@name" />
		</a>
		<xsl:text>&nbsp;/&nbsp;</xsl:text>
	</xsl:template>
