...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="type" mode="fieldgroup-common">
		<div class="panel properties-group">
			...
			<div class="content">
				...
				<xsl:apply-templates select="document(concat('udata://webforms/getAddresses/', @id))/udata" />
				<xsl:choose>
					<xsl:when test="$data-action = 'create'">
						<xsl:call-template name="std-form-buttons-add" />
					</xsl:when>
	...
	<xsl:template match="udata[@module = 'webforms'][@method = 'getAddresses']">
		<div class="field">
			<label>
				<span class="label"><xsl:text>&label-address-send;</xsl:text></span>
				<span>
		...
		<div class="panel properties-group">
			...
			<div class="content">
				...
				<script type="text/javascript">
					var type = new typeControl(<xsl:value-of select="//type/@id" />, {container: "#groupsContainer", form: true});
