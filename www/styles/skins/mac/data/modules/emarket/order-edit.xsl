...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="udata" mode="order-items">
		...
		<div class="panel properties-group">
			...
			<div class="content">
				...
				<div>
					<a href="{$lang-prefix}/admin/emarket/editOrderAsUser/{@id}/">&label-edit-as-user;</a>
					<div>&label-edit-as-user-tip;</div>
				</div>
	...
	<xsl:template match="item" mode="order-items">
		<tr>
			<td>
				<!-- добавлено webeffection -->
				<xsl:variable select="document(concat('uobject://',@id))//property[@name = 'item_link']//page/@parentId" name="item_parent_id" />
				<xsl:variable select="document(concat('upage://', $item_parent_id))/udata" name="item_parent_data" />
				<a href="{$item_parent_data/page/@link}">
					<xsl:value-of select="$item_parent_data//name" />
				</a>
				<xsl:text> | </xsl:text>
				<!-- добавлено webeffection -->
	...
	<xsl:template match="udata[@method = 'ordersList' and count(items/item)]">
		...
		<div class="panel properties-group">
			...
			<div class="content">
				...
				<div>
					<a href="{$lang-prefix}/admin/emarket/actAsUser/{$customer-id}/">&label-act-as-user;</a>
					<div>&label-act-as-user-tip;</div>
				</div>
