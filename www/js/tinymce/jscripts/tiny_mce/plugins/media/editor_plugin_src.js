	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			function isMediaElm(n) {
				return /^mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|FlashVideo|MPEG4)$/.test(n.className);
			};
					...
					each(dom.select('IMG', o.node), function(n) {
						...
						switch (n.className) {
							...
							case 'mceItemFlashVideo':
								...
								cb = 'http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab';
								mt = 'application/x-shockwave-flash';
								break;
		...
		_buildObj : function(o, n) {
			...
			stc = false ; //ed.getParam('media_strict', true) && o.type == 'application/x-shockwave-flash';
			...
			//console.debug(n.className);

			if (/^mceItem(FlashVideo|MPEG4)/.test(n.className)) {
				if (/^mceItemFlashVideo/.test(n.className)) {
					videotype = 'FlashVideo';
				}
				else {
					videotype = 'MPEG4';
				}
				...
				p.flashvars = 'video='+p.src;				
				p.src = '/images/cms/video/UmiVideoPlayer.swf';
				
				if (typeof(p.autoplay)=='boolean' || typeof(p.autoplay)=='string') {
					p.flashvars += '&autoplay='+p.autoplay;
					delete p.autoplay;
				}
				...
				dom.add(ob, 'span', tinymce.extend({_mce_name : 'embed', type : o.type, videotype: videotype, style : dom.getAttrib(n, 'style')}, p));
		...
		_spansToImgs : function(p) {
			...
			each(dom.select('span', p), function(n) {
				...
				if (dom.getAttrib(n, 'class') == 'mceItemObject') {
					if (typeof(dom.getAttrib(n, 'videotype'))=='string' && dom.getAttrib(n, 'videotype')!='') {
						ci = dom.getAttrib(n, 'videotype');
					...
					switch (ci) {
						...
						case 'FlashVideo':
							dom.replace(t._createImg('mceItemFlashVideo', n), n);
							break;

						case 'MPEG4':
							dom.replace(t._createImg('mceItemMPEG4', n), n);
							break;
				...
				if (dom.getAttrib(n, 'class') == 'mceItemEmbed') {
					if (typeof(dom.getAttrib(n, 'videotype'))=='string') {
						type = dom.getAttrib(n, 'videotype');
					}
					else {
		...
		_createImg : function(cl, n) {
			//console.debug('_createImg - ' + cl);
			var im, dom = this.editor.dom, pa = {}, ti = '', args;

			args = ['id', 'name', 'width', 'height', 'videotype', 'bgcolor', 'align', 'flashvars', 'src', 'wmode', 'allowfullscreen', 'quality', 'data'];	
			...
			each(pa, function(v,k) {
				//console.debug('summ '+k+'='+v);
			});

			if (pa['videotype'] && (pa['videotype']=='FlashVideo'||pa['videotype']=='MPEG4')) {
				params = pa['flashvars'].split('&');
