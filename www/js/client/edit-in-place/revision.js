	...
	*/
	this.save = function (callback) {
		...
		if(typeof newValue == 'object') {
			...
			if (jQuery.browser.mozilla && typeof newValue == 'string') {
				if (newValue.match(/src="\.\.\//g)) {
					newValue = newValue.replace(/src="[\.\.\/]+/g, 'src="/');
				}
			}
		...
		jQuery.post(uri + qSuffix, queryParams, function (data) {
			...
			var error = jQuery('error', data);
			var oldLink = jQuery(prop).attr('old-link');
			var newLink = jQuery(prop).attr('new-link');
	...
	*/
	...
	var getNodes = function (returnPlainNodes) {
		...
		for(var i = 0; i < nodes.length; i++) {
			...
			if(node.attr('tagName') == 'TABLE') continue;
			...
			if(info = uPageEditor.searchAttr(originalNode, 20)) {
				if(
	...
	var replaceImage = function (node, value, previousValue) {
		...
		//If image tag
		if(jQuery(node).get(0).tagName == 'IMG') {
			//If image set by src
