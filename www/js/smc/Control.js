	...
	var __DataSetAfterExecute = function(arrData) {
		...
		if (!error) {
			...
			switch (method.toLowerCase()) {
				...
				case 'copy_to_lang' : {
					var i, item, domainId = false;langId  = false; items = hItem.control.selectedList;
					for(i in items) {
						item = items[i];
			...
			if(error_type=='__alias__') {
				...
				elems.each(function() {
					...
					html += " action=''>Страница с таким псевдостатическим адресом уже существует (<a href='"+url+"'>"+url+"</a>): <br/>";
					html += "<br/><label for='new_alias'>" + getLabel('js-content-alias-copy') + "</label> <br/> ";
					html += jQuery(this).attr('path') + "<input type='text' name='new_alias"+jQuery(this).attr('id')+"'  value='"+jQuery(this).attr('alt_name_normal')+"'  id='new_alias"+jQuery(this).attr('id')+"' size='15' />";
					html += "</form>";		
