var uRevisionsQueue = function (editor) {
	var queue = new Array, index = -1, editorInstance = editor, saved = true;
	
	this.submit = function () {
		var submitQueue = new Array(), loaded = 0, i;
		...
		if(submitQueue.length == 0) {
			uPageEditor.message(getLabel('js-panel-message-no-changes'));
			return;
		}
		
		//Save each revision
		if (submitQueue.length) saved = false;
		for(i = 0; i < submitQueue.length; i++) {
			submitQueue[i].save(function () {
				...
				loaded++;
				if (submitQueue.length == loaded) {
					saved = true
					uPageEditor.message(getLabel('js-panel-message-changes-saved'));
				}
			});
		}
		reset();
		...
		if(revision) {
			...
			jQuery.get(uri + qSuffix, info, function (data) {
				...
				if (userType.size() && userType.attr('type') == 'guest') {
					...
					uPageEditor.closeMessages();
					window.session.sessionCloseMessage(true); 
 					return false;
 				}
