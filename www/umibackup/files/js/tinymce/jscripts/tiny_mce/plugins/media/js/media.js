...
function init() {
	...
	if (/mceItem(Flash|FlashVideo|MPEG4|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(ed.dom.getAttrib(fe, 'class'))) {
		pl = fe.title;
	...
	if (pl != "") {
		...
		switch (type) {
			...
			case "flv":
				setBool(pl, 'flv', 'autoplay');
			break;

			case "mpg4":
				setBool(pl, 'mpg4', 'autoplay');
			break;
...
function getType(v) {
	...
	fo = ed.getParam("media_types", "flash=swf;shockwave=dcr;qt=mov,qt,mpg,mp3,mpeg;wmp=avi,wmv,wm,asf,asx,wmx,wvx;rmp=rm,ra,ram;flv=flv;mpg4=mp4").split(';');
...
function changedType(t) {
	...
	d.getElementById('mpg4_options').style.display = 'none';	
...
function serializeParameters() {
	...
	switch (f.media_type.options[f.media_type.selectedIndex].value) {
		...
		case "flv":
			s += getBool('flv', 'autoplay', false);
		break;	

		case "mpg4":
			s += getBool('mpg4', 'autoplay', false);
		break;	
...
function generatePreview(c) {
	...
	if (!tinymce.isIE || document.location.protocol != 'https:') {
		...
		for (n in pl) {
			if (n=='src') {
				if (ourplayer) {
					h += '<param name="src" value="/images/cms/video/UmiVideoPlayer.swf">';
					h += '<param name="flashvars" value="video=' + pl[n] + '">';
				}
				else {
	...
	for (n in pl) {
		if (n=='src' && ourplayer) {
			h += 'flashvars="video=' + pl[n] + '" src="/images/cms/video/UmiVideoPlayer.swf" ';
		}
		else {
			h += n + '="' + pl[n] + '" ';
		}
