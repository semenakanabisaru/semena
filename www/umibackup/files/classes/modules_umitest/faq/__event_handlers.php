		...
		public function confirmUserAnswer($oElement) {
			...
			if ($oElement instanceof umiHierarchyElement && $oElement->getIsActive()) {
				...
				if (umiMail::checkEmail($author_email)) {
					list($sMailSubject, $sMailBody) = def_module::loadTemplates("faq/default", "answer_mail_subj", "answer_mail");
					...
					$sSubject = def_module::parseContent($sMailSubject, $block_arr, $iElementId);
					$sBody = def_module::parseContent($sMailBody, $block_arr, $iElementId);
