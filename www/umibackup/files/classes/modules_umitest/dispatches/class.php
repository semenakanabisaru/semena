		...
		public function unsubscribe() {
			...
			if ($oSubscriber instanceof umiObject) {
				...
				if ($oSubscriberType) {
					...
					else umiObjectsCollection::getInstance()->delObject($iSbsId);
				}
		...
		public function subscribe_do() {
			...
			if($oMdlUsers = $controller->getModule("users")) {
				...
				elseif(umiMail::checkEmail($sSbsMail)) {
					...
					return ($templater instanceof tplTemplater) ? $sResult : Array("result" => $sResult);
				}
			...
			if ($arrSbsDispatches) {
				...
				list($template_mail, $template_mail_subject) = def_module::loadTemplates("dispatches/default", "subscribe_confirm", "subscribe_confirm_subject");
				...
				$mail_subject = def_module::parseContent($template_mail_subject, $mail_arr);
				$mail_content = def_module::parseContent($template_mail, $mail_arr);
