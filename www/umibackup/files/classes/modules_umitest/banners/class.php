...
JS;
		...
		public function insert($sPlace = "", $iMacrosID=0, $bList = false, $current_element_id = false) {
			...
			$sel->where('not_view_pages')->notequals($currentPageId);

			$currentPageParentsIds = umiHierarchy::getInstance()->getAllParents($currentPageId);
			$currentPageParentsIds[] = $currentPageId;

			$sel->option('or-mode')->field('view_pages');
			$sel->where('view_pages')->equals($currentPageParentsIds);
			$sel->where('view_pages')->isnull(true);

			$arrSelResults = array_unique($sel->result);
		...
		protected function sendNotification() {
			...
			foreach ($sel->result() as $banner) {
				...
				if ($days || $views) {
					...
					list($templateLine) = def_module::loadTemplates("mail/banner_notification", "item");
					$itemArr['link'] = $link;
					...
					$items[] = def_module::parseContent($templateLine, $itemArr, false, $bannerId);
			...
			if (count($items)) {
				$blockArr = array();
				list($subject, $template) = def_module::loadTemplates("mail/banner_notification", "subject", "body");
				...
				$subject = def_module::parseContent($subject, $blockArr);
				$mailMessage->setSubject($subject);

				$blockArr['header'] = $subject;
				$blockArr['items'] = $items;

				$content = def_module::parseContent($template, $blockArr);
