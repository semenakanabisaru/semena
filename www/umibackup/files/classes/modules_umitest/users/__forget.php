<?php
	abstract class __forget_users {
		...
		public function forget_do($template = "default") {
			...
			) = def_module::loadTemplates("users/forget/".$template,
			...
			if ($user_id) {
				...
				if ($without_act || intval($object->getValue('is_activated'))) {
					...
					$mail_subject = def_module::parseContent($template_mail_verification_subject, $mail_arr, false, $user_id);
					$mail_content = def_module::parseContent($template_mail_verification, $mail_arr, false, $user_id);
		...
		public function restore($template = "default", $activate_code = false) {
			static $result = Array();
			...
			if($user_id && $activate_code) {
				...
				$mail_subject = def_module::parseContent($template_mail_password_subject, $mail_arr, false, $user_id);
				$mail_content = def_module::parseContent($template_mail_password, $mail_arr, false, $user_id);
