				...
				foreach($dirs as $dirName) {
					if(is_dir($dirName)) {
						...
						foreach ($files as $path => $name) {
							...
							if(!is_dir($destination . ltrim($file->getDirName(), '.'))) mkdir($destination . ltrim($file->getDirName(), '.'), 0777, true);
				...
				while($continue){
					if ($reader->nodeType == XMLReader::ELEMENT) {
						...
						if($node_name != 'umidump') {
							...
							$writer->writeRaw($node_value);
							if($dom->getElementsByTagName($node_name)->item(0)->hasChildNodes() && $node_name != 'meta') {
