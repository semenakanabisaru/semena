	...
	class backupModel extends singleton implements iBackupModel {
		...
		*/
		...
		/**
			...
			* @return Array список изменений
		*/
		public function getAllChanges($cparam = false) {
			if (!regedit::getInstance()->getVal("modules/backup/enabled")) {
				return false;
			}
			...
			$sql = "SELECT param, id, ctime, changed_module, user_id, is_active FROM cms_backup WHERE (" . time() . "-ctime)<" . $end_time . " ORDER BY ctime DESC";
			$result = l_mysql_query($sql);
			...
			$params = array();
			$rows = array();
			while(list($element_id, $revision_id, $ctime, $changed_module, $user_id, $is_active) = mysql_fetch_row($result)) {
