	...
	class umiHierarchyElement extends umiEntinty implements iUmiEntinty, iUmiHierarchyElement {
		...
		*/
		...
		public function updateYML() {
			...
			if ($this->is_active && !$this->is_deleted && $this->getValue('price')) {
				...
				if (!$templater instanceof xslTemplater) {
					...
					$templater = $controller->setCurrentTemplater(array(
						'class_name' => 'xslTemplater',
						'type'       => 'xslt',
						'dir_path'   => CURRENT_WORKING_DIR . '/xsl/export/',
						'file_path'  => $style_file
					));
					$templater->init();
				}
				else {
					$old_path = $templater->getFilePath();
				...
				elseif (isset($old_path)) {
					$templater->setFilePath($old_path);
				}
