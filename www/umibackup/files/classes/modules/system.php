...
<script type="text/javascript">
	var _gaq = _gaq || [];
	_gaq.push(
	    ['_setAccount', '{$googleAnalyticsId}'],
	    ['_setDomainName', '{$domain->getHost()}'],
...
END;
	...
	public function  social_networks(){
		...
		$result['vkontakte'] = Array(
			"attribute:url_success"			=> 'http://'.getServer('HTTP_HOST').'/vkontakte_cb?act=success',
			"attribute:url_fail"			=> 'http://'.getServer('HTTP_HOST').'/vkontakte_cb?act=fail',
			"attribute:currency"			=> 'RUB',
			"attribute:wishlist_enabled"	=> (int) $regedit->getVal('//modules/emarket/vkontakte_wishlist'),
			"attribute:order_enabled"		=>(int) $regedit->getVal('//modules/emarket/vkontakte_order'),
			"attribute:shop_enabled"		=>(int) $regedit->getVal('//modules/emarket/vkontakte_shop_enabled'),
			"attribute:merchant_id"			=>(int) $regedit->getVal('//modules/emarket/vkontakte_merchant_id'),
			"attribute:key"					=>(string) $regedit->getVal('//modules/emarket/vkontakte_key'),
		);
