<?php
	class testHost {
			...
			if ($link && $db){
				...
				*/
				...
				if ($this->mode == 'migration') {
					$result = mysql_query("SHOW tables LIKE 'cms3_%'");

					$i=0;
					while($row2 = mysql_fetch_array($result)) {
						$row3 = mysql_fetch_array(mysql_query("SHOW CREATE TABLE `{$row2[0]}`"));
						$this->assert(strpos($row3[1], 'ENGINE=InnoDB') != 0, 13017, true, $row3[0]);
					}
				}
			}
		}
		...
		*/
		...
		function setConnect($host, $user, $password, $database, $mode) {
			$this->user = $user;
			$this->host = $host;
