                ...
                <div id='sidebar_menu'>
                    <a href='/producers/' class='button'><div umi:element-id="28" umi:field-name="name">Каталог</div></a>
                        ...
                        <div class='body last'> 
                            ...
                            <div class='scope'>
                                <div class="title"><div><span>По содержанию ТГК</span></div></div>
                                <ul id='tgk_list'>
                                    <li>
                            ...
                            <div class='scope'>
                                <div class="title"><div><span>По стоимости</span></div></div>
                                <ul id='price_list'>
                                    <li>
...
<xsl:template match="user" mode="user_menu">
	<li><a href='/working/' class='points'>Мои скидки</a></li> <!-- /users/registrate/ -->
	<li><a href='/working/' class='orders'>Мои заказы</a></li> <!-- /users/registrate/ -->
...
<xsl:template match="user" mode="user_auth">
	<li class='enter'><a href="/working/" class='item'>Войти</a></li> <!-- /users/auth/ -->
	<li class="delimiter small"></li>
	<li><a href="/working/" class='item'>Регистрация</a></li> <!-- /users/registrate/ -->
</xsl:template>
...
<xsl:template match="udata[@module = 'catalog'][@method = 'getCategoryList']" mode='fproducers'>
    <div class='scope last'>
        <div class="title"><div><span>По производителю</span></div></div>
...
<xsl:template match="group" mode='fcats' >
    <div class='scope'>
        <div class='title'><div><span>По категории</span></div></div>
        <ul id='cats_list'>
...
<xsl:template match="group" mode='ftypes'>
    <div class='scope'>
        <div class="title"><div><span>По типу</span></div></div>
        <ul id='types_list'>
        ...
        <div id="personal">
            ...
            <xsl:choose>
                <xsl:when test="user/@status = 'auth'">
                    ...
                    <p class="tip">Вы находитесь в разделе "Моя Корзина" где показаны все товары добавленные вами в Корзину. Здесь вы можете изменить количество заказываемых товаров, убедиться в верности сделанного выбора, уточнить промежуточные и итоговые стоимости. 
                    Для продолжения покупки и перехода к "Оформлению заказа" нажмите соответствующую кнопку</p>  
                    ...
                    <div class="form_common" id='order_forming'>
                        ...
                        <ul>
                            ...
                            <xsl:apply-templates select="document(concat('udata://data/getEditForm/',$user_id,'/(adres_dostavki)'))//field" mode="adres_edit_form" >
                                <xsl:width-param name="user_data" value="$user_data"/>
                            </xsl:apply-templates>
                        </ul>
                        </form>
                    </div>
...
<xsl:template match="udata[@module = 'emarket'][@method = 'ordersListMy']">
    <xsl:choose>
        <xsl:when test="//item">
            ...
            <p class="tip">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Fusce nec quam mi, id mattis urna. Nam pretium vehicula felis vel scelerisque. Cras eget est odio, in egestas augue.</p>
...
<xsl:template match="udata[@module = 'emarket'][@method = 'orderMy']">
    ...
    <table class="my_order_table">
        ...
        <tr class='total'>
            <td colspan='4'>
                Общая сумма: <span><xsl:value-of select=".//summary//actual" /> руб.</span>
            </td>
        </tr>
...
<xsl:template match="item" mode="my_order_item" >
    <tr class='item'>
        ...
        <td class='border_r'><xsl:value-of select="document(concat('upage://', page/@parentId))/udata/page/name" /></td>
    ...
    <div id="view_good">
        <div class="photos">
            ...
            <div class="image">
                ...
                <a class='zoom_img fancy' href="{$original_image}">
...
<xsl:template match="udata[@module = 'catalog'][@method = 'getObjectsList']" mode='packages'>
    <xsl:choose>
        ...
        <xsl:otherwise>
            <xsl:text>Нет ничего на складе.</xsl:text>
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
